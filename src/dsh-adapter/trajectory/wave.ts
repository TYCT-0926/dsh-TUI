 ...
 */
function weightOf(node: TrajNode): number {
  if (node.kind === 'turn' || node.kind === 'step') return 0.25
  if (node.burst !== undefined) return Math.min(4, node.burst.members.length)
  return 1
}
...
export function projectWave(
  ...
  if (width === 0 || nodes.length === 0) return { buckets: [], peak: 0, turns: [] }
  ...
  let peak = 0
  for (const slot of buckets) if (slot.weight > peak) peak = slot.weight
