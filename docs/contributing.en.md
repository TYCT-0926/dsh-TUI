...
- Leave `declarationMap` off; do not turn it back on. A declaration map exists
  ...
  `src/` (`npm pack` measures zero `src/` entries), so the maps point at paths
  the tarball does not contain and the jump cannot resolve. The cost is real,
  though: a `.d.ts.map` is one single line holding the whole file's coordinate
  table, rewritten end to end by any source edit, and git merges by line — two
  PRs touching the same module always collide on that line, and the collision
  is only resolvable by rebuilding, never by hand.
