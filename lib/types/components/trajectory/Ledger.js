...
export function Ledger({ rows, start, height, cursor, width, tick, arrivalTick, arrivalFrom, }) {
    ...
    return (_jsx(Box, { flexDirection: "column", flexGrow: 1, flexShrink: 1, overflow: "hidden", children: visible.map((node, offset) => {
            ...
            if (node.kind === 'turn') {
                ...
                return (_jsx(Box, { width: "100%", height: 1, flexShrink: 0, children: _jsxs(Text, { color: tone, bold: focused, children: [`${focused ? '▸' : ' '}── ${node.label}`, _jsx(Text, { color: "subtle", children: idleText }), _jsx(Text, { color: "subtle", children: ` ${'─'.repeat(fill)} ${right}` })] }) }, `${node.seq}:turn`));
            }
