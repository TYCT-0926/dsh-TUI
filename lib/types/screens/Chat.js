...
import { AlternateScreen, Box, Text, useInput, ScrollBox, useTheme } from '../ui.js';
...
import { extendTrajectory } from '../dsh-adapter/trajectory/index.js';
...
export function Chat({ channel, questionStore, approvalStore, onExit, onUpdate, fullscreen = false, }) {
    ...
    React.useEffect(() => {
        if (unreadFailures === 0 || noticedFailureRef.current || sceneOpen)
            return;
        noticedFailureRef.current = true;
        channel.notify(t('traj-notice-failure', { n: unreadFailures, key: `${modLabel}t` }), { color: 'warning', timeoutMs: 6000 });
