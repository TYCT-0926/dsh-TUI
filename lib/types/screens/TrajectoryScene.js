...
import { aggregate, columnOfIndex, extendTrajectory, inspectNode, projectWave, } from '../dsh-adapter/trajectory/index.js';
...
export function TrajectoryScene({ channel, onClose, }) {
    ...
    const [projection, setProjection] = React.useState('sequence');
    ...
    // giving up a row.
    const header = (_jsxs(Box, { flexDirection: "row", width: "100%", gap: 2, flexShrink: 0, children: [_jsx(Box, { flexShrink: 0, children: _jsxs(Text, { color: "claude", bold: true, children: ["\u2726 ", t('traj-title')] }) }), _jsx(Box, { flexGrow: 1, flexShrink: 1, overflow: "hidden", children: _jsx(Text, { wrap: "truncate", color: "subtle", children: channel.sessionTitle ?? channel.cwd }) }), _jsx(Box, { flexShrink: 0, children: _jsxs(Text, { color: "subtle", children: [t('traj-totals', { turns: totals.turns, steps: totals.rows }), totals.errors > 0 ? _jsx(Text, { color: "error", children: ` · ${t('traj-errors', { n: totals.errors })}` }) : '', totals.retries > 0 ? _jsx(Text, { color: "warning", children: ` · ${t('traj-retries', { n: totals.retries })}` }) : '', ` · ${formatDuration(totals.spanMs)}`] }) })] }));
    const tabs = (_jsxs(Box, { flexDirection: "row", width: "100%", gap: 2, flexShrink: 0, children: [_jsxs(Box, { flexShrink: 0, flexDirection: "row", gap: 2, children: [_jsxs(Text, { color: view === 'timeline' ? 'permission' : 'subtle', bold: view === 'timeline', children: [view === 'timeline' ? '●' : '○', " ", t('traj-tab-timeline')] }), _jsxs(Text, { color: view === 'hotspot' ? 'permission' : 'subtle', bold: view === 'hotspot', children: [view === 'hotspot' ? '●' : '○', " ", t('traj-tab-hotspot')] })] }), _jsx(Box, { flexGrow: 1, flexShrink: 1, overflow: "hidden", children: queryOpen || !query.empty ? (_jsxs(Text, { wrap: "truncate", children: [_jsx(Text, { color: "permission", children: "/ " }), _jsx(Text, { color: "suggestion", children: queryText }), queryOpen ? _jsx(Text, { color: "subtle", children: "\u258C" }) : '', _jsx(Text, { color: "subtle", children: `  ${t('traj-matches', { n: filtered.length, total: nodes.length })}` })] })) : (_jsx(Text, { children: " " })) }), _jsx(Box, { flexShrink: 0, children: _jsx(Text, { color: "subtle", children: view === 'hotspot' ? t(`traj-sort-${sort}`) : t(`traj-proj-${projection}`) }) })] }));
    ...
    return (_jsxs(Box, { ref: ref, flexDirection: "column", width: "100%", paddingX: 1, children: [header, tabs, _jsx(WaveBand, { band: band, width: bandWidth, cursorColumn: columnOfIndex(band, indexes[clampedCursor] ?? 0), viewportStart: columnOfIndex(band, indexes[windowStart] ?? 0), viewportEnd: columnOfIndex(band, indexes[Math.min(filtered.length - 1, windowStart + ledgerRows - 1)] ?? 0), matches: matchColumns, tick: tick, alertTick: alertTick }), view === 'timeline' ? (_jsxs(_Fragment, { children: [_jsx(Ledger, { rows: filtered, offsets: indexes, start: windowStart, height: ledgerRows, cursor: clampedCursor, width: columns - 2, tick: tick, arrivalTick: arrivalTick, arrivalFrom: arrivalFrom }), _jsx(Divider, { color: "permission", width: bandWidth }), _jsx(Inspector, { node: focused, detail: detail, height: inspectorRows, width: columns - 2, expanded: expanded, scroll: inspectScroll })] })) : (_jsx(HotspotView, { agg: agg, sort: sort, width: columns - 2, height: ledgerRows + inspectorRows + 1, cursor: hotCursor, tick: tick, switchTick: switchTick })), _jsx(Box, { width: "100%", flexShrink: 0, children: _jsxs(Text, { dimColor: true, italic: true, wrap: "truncate", children: [_jsx(HintLine, { text: hints }), totals.tokens.input > 0 ? (_jsx(Text, { color: "subtle", children: `   ${formatTokens(totals.tokens.input)}→${formatTokens(totals.tokens.output)}` })) : ('')] }) })] }));
}
